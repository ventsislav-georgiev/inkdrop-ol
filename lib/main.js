  ...
  getLayers() {
    ...
    if (this.config.markers && this.config.markers.items.length) {
      const iconFeatures = this.config.markers.items.map(item => {
        const iconFeature = new ol.Feature({ geometry: new ol.geom.Point(ol.proj.fromLonLat(item.location.reverse())) });
        const { items: _, ...markerStyle } = this.config.markers
        const iconStyle = new ol.style.Style({ image: new ol.style.Icon({ ...markerStyle, ...item }) }); // Destruct markers? Add custom marker props?
        iconFeature.setStyle(iconStyle);
        return iconFeature
      })
