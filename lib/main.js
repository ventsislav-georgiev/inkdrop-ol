...
class OpenLayers extends React.Component {
  ...
  parseConfig() {
    ...
    this.config = _.merge({}, defaults, userConfig, {
      ...
      markers: _.merge({}, defaults.markers, userConfig.markers),
  ...
  createMap() {
    ...
    this.map.on("click", (e) => {
      const popupFeature = this.map.forEachFeatureAtPixel(e.pixel, feature => feature);
      if (!popupFeature) {
        this.popup.hide();
        return;
      }

      const popupContent = popupFeature.get('popupContent');
      if (popupContent) {
        this.popup.show(e.coordinate, popupContent);
      }
    });
  ...
  updateMap() {
    ...
    if (this.config.markers.hidePopupOnUpdate && this.popup && this.popup.isOpened()) {
      this.popup.hide();
    }
  }
  ...
  getLayers() {
    ...
    if (this.config.markers && this.config.markers.items?.length) {
      const iconFeatures = this.config.markers.items.map(item => {
        const iconFeature = new ol.Feature({
          geometry: new ol.geom.Point(ol.proj.fromLonLat(item.location.reverse())),
          popupContent: item.popupContent
        });
        const { items: _, ...markerStyle } = this.config.markers;
        const iconStyle = new ol.style.Style({ image: new ol.style.Icon({ ...markerStyle, ...item }) });
        iconFeature.setStyle(iconStyle);
        return iconFeature;
      });
