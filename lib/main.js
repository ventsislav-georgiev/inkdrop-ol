...
class OpenLayers extends React.Component {
  ...
  updateMap() {
    ...
    if (!this.olgm && this.isgmaps) {
      ...
      this.olgm = new OLGoogleMaps({
        map: this.map,
        gmapOptions: {
          mapTypeId: this.config.gmaps.mapType
        }
      });
    }
    ...
    if (this.config.gmaps.traffic && this.trafficLayer) this.trafficLayer.setMap(this.olgm.getGoogleMapsMap());
  }
  ...
  getLayers() {
    ...
    return layers.concat(layerSources.map(source => new ol.layer.Tile({ source })));
  }
}
